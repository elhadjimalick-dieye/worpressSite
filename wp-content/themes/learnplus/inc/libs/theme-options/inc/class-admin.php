<?php
/**
 * TrendingTemplates Options Framework
...
class LearnPlus_Theme_Options_Admin {
	...
	public function __construct( $options ) {
		...
		add_action( 'wp_ajax_trending_template_options_save', array( $this, 'save' ) );
		add_action( 'wp_ajax_trending_template_options_reset', array( $this, 'reset' ) );
		add_action( 'wp_ajax_trending_template_options_import', array( $this, 'import' ) );
		add_action( 'wp_ajax_trending_template_generate_custom_css', array( $this, 'custom_css' ) );
	}
	...
	public function admin_menu() {
		$hook = add_theme_page( esc_html__( 'Theme Options', 'learnplus' ), esc_html__( 'Theme Options', 'learnplus' ), 'edit_theme_options', 'theme-options', array( $this, 'page' ) );
	...
	public function enqueue() {
		...
		wp_localize_script(
			...
			'nonce_save'        => wp_create_nonce( 'trending_template_options_save' ),
			'nonce_reset'       => wp_create_nonce( 'trending_template_options_reset' ),
			'nonce_import'      => wp_create_nonce( 'trending_template_options_import' ),
			'nonce_css'         => wp_create_nonce( 'trending_template_generate_custom_css' ),
			'reset_notice'      => esc_html__( 'This action can\'t be undo. Are you sure want to reset?', 'learnplus' ),
			'import_notice'     => esc_html__( 'All previous data will be overwritten and can\'t be undo. Are you sure want to import?', 'learnplus' ),
			'media_title'       => esc_html__( 'Select Image', 'learnplus' ),
			'media_title_multi' => esc_html__( 'Select Images', 'learnplus' ),
		)
		);
	}
	...
	public function save() {
		check_ajax_referer( 'trending_template_options_save' );
	...
	public function reset() {
		check_ajax_referer( 'trending_template_options_reset' );
		foreach ( LearnPlus_Theme_Options::$defaults as $name => $default ) {
	...
	public function import() {
		check_ajax_referer( 'trending_template_options_import' );
		$func = 'base64' . '_decode';
		$data = maybe_unserialize( $func( $_POST['data'] ) );
	 ...
	 * By default, it does nothing, just define a hook called 'trending_template_generate_custom_css'.
	 *
	 * @since 1.0
	 *
	 * @return void
	...
	public function custom_css() {
		check_ajax_referer( 'trending_template_generate_custom_css' );
