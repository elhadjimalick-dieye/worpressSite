...
function learnplus_theme_option_fields() {
	...
	// Help information
	$options['help'] = array(
		'document' => 'http://trendingtemplates.com/docs/learnplus/',
		'support'  => 'http://trendingtemplates.com/support/learnplus/',
	);
	...
	$options['fields']['lms'] = array(
		...
		array(
			'name'    => 'course_comment',
			'label'   => esc_html__('Course comment', 'learnplus'),
			'desc'    => esc_html__('Display comment form on course page', 'learnplus'),
			'type'    => 'switcher',
			'default' => 1,
		),
	);
...
function learnplus_generate_custom_color_scheme() {
	...
	$less = $wp_filesystem->get_contents(get_template_directory() . '/css/color-schemes/mixin.less');
	$less .= ".custom-color-scheme { .color-scheme($color_1, $color_2); }";

	// Compile
	require get_template_directory() . '/inc/libs/lessc.inc.php';
	$compiler = new lessc;
...
function learnplus_theme_options_scripts($hook) {
	...
	wp_enqueue_script('learnplus-admin', get_template_directory_uri() . "/js/backend/options$min.js", array('jquery'), '20160712', true);
}
