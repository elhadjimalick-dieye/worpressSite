...
function learnplus_meta_boxes_scripts( $hook ) {
	...
	wp_enqueue_style( 'admin', get_template_directory_uri() . "/css/backend/admin.css", array(), '20160712' );
	// Detect to load un-minify scripts when WP_DEBUG is enable
	$min = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';

	if ( in_array( $hook, array( 'post.php', 'post-new.php' ) ) ) {
		wp_enqueue_script( 'learnplus-meta-boxes', get_template_directory_uri() . "/js/backend/meta-boxes$min.js", array( 'jquery' ), '20160712', true );
	}
}
