            ...
            if ( isset($wp_rem_cs_section_background_option) && $wp_rem_cs_section_background_option == 'section-custom-background-image' ) {
                ...
                if ( $url['host'] == $_SERVER["SERVER_NAME"] ) {
                  ...
                  //  $section_video_element = wp_oembed_get($wp_rem_cs_section_video_url, array( 'height' => '1083' ));
                    if ( isset($wp_rem_cs_section_video_url) && ! empty($wp_rem_cs_section_video_url) ) {

                        $url_parts = parse_url($wp_rem_cs_section_video_url);
                        $url_vid = '';
                        if ( isset($url_parts['query']) && ! empty($url_parts['query']) ) {
                            $url_vid = $url_parts['query'];
                            $url_vid = str_replace('v=', '', $url_vid);
                        }
                        $player_string_exists = strpos($wp_rem_cs_section_video_url, 'vimeo.com');
                        if ( $player_string_exists === true ) {
                            $video_path_exists = strpos($url_parts['path'], 'video');
                            if ( $video_path_exists === true ) {
                                $wp_rem_cs_section_video_url = 'https://player.vimeo.com/' . $url_parts['path'] . '';
                            } else {
                                $wp_rem_cs_section_video_url = 'https://player.vimeo.com/video/' . $url_parts['path'] . '';
                            }
                        }
                        $section_video_element = wp_oembed_get($wp_rem_cs_section_video_url, array( 'height' => '1083' ));
                        if ( $wp_rem_cs_section_video_autoplay == 'yes' ) {
                ...
                <div class="<?php echo sanitize_html_class($wp_rem_cs_section_view); ?> "> 
                    ...
                    if ( isset($section_video_element) && ! empty($section_video_element) ) {
                        ...
                        <div class="video-holder">
                            ...
                            ?>
                            <?php if ( isset($section_video_element) && ! empty($section_video_element) ) { ?>
                            </div>
                        </div>
                    <?php } ?>
                </div>
