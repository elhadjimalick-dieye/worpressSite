...
/*
 * CS_ICONS_MANAGER_STABLE_VERSION contains cs framework stable version compitble with this theme version
 */
if ( ! defined( 'CS_ICONS_MANAGER_STABLE_VERSION' ) ) {
    define( 'CS_ICONS_MANAGER_STABLE_VERSION', '1.1' );
}
...
if ( ! defined( 'DEMO_DATA_HOME_URL' ) ) {
    define( 'DEMO_DATA_HOME_URL', 'http://homevillas.chimpgroup.com/{{{demo_data_name}}}' );
}
...
if ( ! function_exists( 'get_server_requirements' ) ) {
    ...
    function get_server_requirements() {
        ...
        $server_requirements = array(
			...
			array(
                'title' => 'cURL Support '. wp_rem_cs_var_theme_text_srt('wp_rem_cs_var_config_should_be_enabled') .' php.ini',
                'description' => wp_rem_cs_var_theme_text_srt('wp_rem_cs_var_config_download_import_data'),
                'version' => '',
                'is_met' => extension_loaded('curl'),
            ),
			array(
                'title' => 'Zip '. wp_rem_cs_var_theme_text_srt('wp_rem_cs_var_config_should_be_enabled') .' php.ini',
                'description' => wp_rem_cs_var_theme_text_srt('wp_rem_cs_var_config_download_import_data'),
                'version' => '',
                'is_met' => extension_loaded('zip'),
            ),
			array(
                'title' => 'Json Support '. wp_rem_cs_var_theme_text_srt('wp_rem_cs_var_config_should_be_enabled') .' php.ini',
                'description' => wp_rem_cs_var_theme_text_srt('wp_rem_cs_var_config_download_import_data'),
                'version' => '',
                'is_met' => extension_loaded('json'),
            ),
			array(
                'title' => 'XML Support '. wp_rem_cs_var_theme_text_srt('wp_rem_cs_var_config_should_be_enabled') .' php.ini',
                'description' => wp_rem_cs_var_theme_text_srt('wp_rem_cs_var_config_download_import_data'),
                'version' => '',
                'is_met' => extension_loaded('xml'),
            ),
			array(
                'title' => 'UPLOADS_PERMISSIONS = ' . $recommended_upload_permission . ' ( '. wp_rem_cs_var_theme_text_srt('wp_rem_cs_var_config_available') .' ' . $upload_permission . ' )',
                'description' => wp_rem_cs_var_theme_text_srt('wp_rem_cs_var_config_download_import_attachments'),
                'version' => '',
...
if ( ! function_exists( 'get_plugin_requirements' ) ) {
    ...
    function get_plugin_requirements() {
        ...
        $compatible_plugin_versions = array(
			...
			'cs_icons_manager' => CS_ICONS_MANAGER_STABLE_VERSION,
        );
		...
		// Check if there is a need to prompt user to install theme.
        $is_cs_icons_manager = class_exists( 'CS_Icons_Manager' );
        $current_version_cs_icons_manager = '0.0';
        $have_new_version_cs_icons_manager = false;
        if ( $is_cs_icons_manager ) {
            $current_version_cs_icons_manager = CS_Icons_Manager::get_plugin_version();
            $new_version_cs_icons_manager = $compatible_plugin_versions['cs_icons_manager'];
            if ( version_compare( $current_version_cs_icons_manager, $new_version_cs_icons_manager ) < 0 ) {
                $is_cs_icons_manager = false;
                $have_new_version_cs_icons_manager = true;
            }
        }
        ...
        $plugin_requirements = array(
			...
			'cs_icons_manager' => array(
                'title' => wp_rem_cs_var_theme_text_srt('wp_rem_cs_var_config_icons_manager_title'),
                'description' => wp_rem_cs_var_theme_text_srt('wp_rem_cs_var_config_icons_manager_desc'),
                'version' => $current_version_cs_icons_manager,
                'new_version' => ( true == $have_new_version_cs_icons_manager ) ? $new_version_cs_icons_manager : '',
                'is_installed' => $is_cs_icons_manager,
            ),
			'rev_slider' => array(
...
if ( ! function_exists( 'get_mandaory_plugins' ) ) {
	function wp_rem_file_perms($file, $octal = false) {
